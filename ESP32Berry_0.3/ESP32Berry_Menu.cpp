  ...
  if (code == LV_EVENT_CLICKED) {
    if (obj == uiMenuCloseBtn) {
      ...
      switch (lv_obj_get_child_id(obj)) {
        case 0:
          _display->ui_popup_open("ESP32Berry v0.3", "ESP v2.0.4\nLVGL v8.3.1\nLovyanGFX v0.4.18\nFastBot v2.22\n\nby ThatProject");
          break;
        case 2:
...
void Menu::wifi_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    String selectedItem = lv_list_get_btn_text(uiWiFiList, obj);
    String selectedSSID = selectedItem.substring(0, selectedItem.indexOf(" (-"));
    lv_label_set_text(mboxTitle, selectedSSID.c_str());
...
void Menu::open_menu_sdcard() {
  if (!_system->is_SDcard_available()) {
    _display->ui_popup_open("Oops!", "Check Your SD Card.");
    return;    
  }
