...
AppNote::AppNote(Display *display, System *system) {
  instance = this;
  _display = display;
  _bodyScreen = display->getBodyScreen();
  _system = system;
  ...
  if (!_system->createDir(NOTE_PATH)) {
    // Todo: error popup
  }
}
...
void AppNote::event_handler(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    if (obj == closeBtn) {
      this->closeApp();
    } else if (obj == uiFileListCloseBtn) {
      this->ui_close_file();
      ...
      if (newFilename.length() == 0) {
        ...
        if (_system->writeFile(filePath.c_str(), text.c_str())) {
...
void AppNote::file_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    instance->contents = _system->readFile(filePath.c_str());
...
void AppNote::ui_app() {
  ...
  lv_obj_add_event_cb(closeBtn, event_handler_thunk, LV_EVENT_CLICKED, NULL);

  lv_obj_t *label = lv_label_create(closeBtn);
  lv_label_set_text(label, LV_SYMBOL_CLOSE);
  lv_obj_center(label);

  lv_obj_t *appTitle = lv_label_create(appMain);
  lv_label_set_text(appTitle, "Note App");
...
void AppNote::ui_open_file() {
  ...
  std::vector<String> noteFileList = _system->listDir(NOTE_PATH);
